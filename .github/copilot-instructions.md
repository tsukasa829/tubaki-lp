...
# Copilot Behavior Instructions (Claude Usage Reduction)

以下は Copilot が従うべき指示です。(Claude Usage Reduction)

以下の指示に従って動作してください。

## モデル選択

* Claude 系モデル（Claude 3.5 など）は必要不可欠な場合のみ使用してください。
* 可能な限り GPT シリーズ（GPT-4.1 / GPT-5 / GPT-4.1 mini）を優先してください。

## タスク制御

* 大規模レビュー、長文要約、広範囲のリファクタリングが必要な場合でも、まずは GPT 系モデルで対応してください。
* ローカルなコード修正、軽量生成タスクを優先し、抽象的な議論は分割して扱ってください。

## コンテキスト最適化

* 不要に大量のコンテキストを読み込まないでください。
* 開いているファイルや差分の範囲は必要最小限に抑えてください。

## Diff / レビュー動作

* ファイル全体や大きな差分より、対象範囲を限定してレビューしてください。
* コード改善時は局所的な変更を優先してください。

## 自動アクション

* 重い推論が必要な自動修正や広範囲解析は、可能な限り避けてください。

## 応答姿勢

* 抽象化や高度推論が不要な場合、軽量なモデル動作に徹してください。
* Claude を呼び出す必要がある場合は、その理由が明確なときのみに限ってください。
